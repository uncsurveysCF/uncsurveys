...
nom <- paste('../Datos/img/grafMannWhitney_',idP1,idOp1,idP2,'.png',sep='')
png(file=nom)
...
dev.off()
datosPon <- as.numeric(datos$Ponderacion)
pw <- wilcox.test(jitter(as.integer(Ponderacion))~grupos,data=datos) 
pw
dbDisconnect(con)