...
sql <- paste("SELECT CONCAT(O.idOpcion,' - ',O.Texto,'##') AS Texto, (SELECT COUNT(*) FROM respuestaspreguntas R WHERE R.idOpcion = O.idOpcion) AS Cantidad from opcionespreguntas O where O.idPregunta = ",idP,sep='')
