...
$(document).ready(function () {
        ...
        $.post("Negocios/encuestas.php", {idEncuesta: $("#hfIdEncuesta").val(), oper: 'verEncuesta'}, function (data) {
            ...
            if (isNaN(respuesta)) {// como no es un numero, significa que devolvio un json
                ...
                if ((datosEnc.FechaLimite != "") && (datosEnc.FechaLimite != null)) {
                    ...
                    $("#txtFcFin").val(datosEnc.FechaLimite);                    
                } else {
 ...
 $("#formrecopilar").validate({
        ...
        var seguir = true;
        var tipoEnc = parseInt($("#tipoEncuesta").val());
        if(tipoEnc >= 2){
            if(($("#txtEnlaceWeb").val() == "") && ($("#txtIdsPart").val() == ""))
            {
                $("#respuesta").addClass( "alert alert-danger" );
                $("#respuesta").html("Debe Ingresar una Identificaci&oacute;n de los participantes o habilitar el recopilador web");
                seguir = false;
            }
        }   
        if(seguir){
